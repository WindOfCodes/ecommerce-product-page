...
const smallPhotos = Array.from(document.querySelectorAll(".photo_small"));
const smallImg = Array.from(document.querySelectorAll(".photoImg"));
...
prevBtn.forEach((pBtns) => {
  pBtns.addEventListener("click", function () {
    ...
    smallPhotos[currentMiniatureId].classList.add("borderActive");
    smallImg[currentMiniatureId].classList.add("opacityActive");
    // `img${currentImgId}`.classList.add("opacityActive");
    // smallImg.src.contains(currentImgId).classList.add("opacityActive");
    // dla smallImg któego src zawiera currentImgId dodaj klasę opacityActive
  });
});
